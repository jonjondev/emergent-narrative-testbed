...
static func generate_current_state(owner):
	...
	current_state[StateConditions.IS_SAFE] = not (any_fires(owner) or any_intruders(owner))
...
static func any_fires(owner) -> bool:
...
static func any_intruders(owner) -> bool:
	if owner.blackboard.get("intruder"):
		for intruder in owner.blackboard.get("intruder"):
			if intruder.present:
				return true
	return false
